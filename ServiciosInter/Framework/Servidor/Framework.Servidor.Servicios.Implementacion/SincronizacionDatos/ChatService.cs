...
namespace Framework.Servidor.Servicios.Implementacion.SincronizacionDatos
{
    ...
    internal class ChatService : ScsService, IChatService
    {
        ...
        /// </summary>
        private static readonly ThreadSafeSortedList<long, ChatClient> _clients = new ThreadSafeSortedList<long, ChatClient>();
        ...
        public void Login(Hik.Samples.Scs.IrcChat.Arguments.UserInfo userInfo)
        {
            //Check nick if it is being used by another user
            if (FindClientByNick(userInfo.Nick) != null)
            {
                
               var cliente = _clients.GetAllItems().Where(c => c.User.Nick == userInfo.Nick).FirstOrDefault();
               if (cliente != null)
               {
                   ClientLogout(cliente.Client.ClientId);
               }
